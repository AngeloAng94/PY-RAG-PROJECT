...
**Stato**: scaffolding completo e revisionato · 54 test verdi (offline) · indurito per repo reali (timeout, split, resilienza, auto-skip asset, progress feedback) · nodo esposto con doppio nome `retrieve`/`retrieve_context`
...
│       ├── test_indexer.py       # 11 test (resilienza, auto-skip asset, exclude/include, progress)
...
| `rag/indexer.py` | 403 | Ingest offline: `index_file`, `index_repo`, `infer_layer`, fallback `ABSENT`; **auto-skip asset content-based** (`looks_like_data`), glob `--exclude`/`--include`, **resilienza per-file**; **progress feedback** (pre-scan `found N files…`, riga per file `[i/N] indexing/skipped …`, `progress=True` di default); summary `skipped_data`/`skipped_error`/`skipped_excluded`. |
...
| **Totale codice `rag/` + `scripts/`** | **2334** | — |
| **Totale `tests/rag/`** | **1220** | 54 funzioni di test |
| **Totale complessivo** | **3554** | (incl. file `__init__`) |
...
Suite eseguita offline con `FakeEmbedder` deterministico (nessun runtime/rete). Esito: **54 passed** (1 warning di deprecation proveniente da OpenTelemetry/Chroma, non dal codice del progetto).
...
| Indexer (`test_indexer.py`) | 11 | PASS — split persistito con id distinti; **resilienza** (file fallito saltato, build continua); **auto-skip asset** (riga lunga / byte-array); modulo normale indicizzato; `--include`/`--include-data`/`--exclude`; summary `skipped_data`/`skipped_error` separati; **progress feedback** (pre-scan `found N…`, righe `[i/N]`, `progress=False` silenzia) |
...
| 13 | field feedback | Progress feedback in `index_repo` (feedback dal campo: build >1h senza output, impossibile distinguere lavoro da hang): pre-scan che stampa `found N files to index, M excluded`, riga per file `[i/N] indexing <rel> (chunks=k)` / `[i/N] skipped <rel> (data|error)` con `flush=True`, param `progress=True` di default (`progress=False` per run silenziosi). `scripts/build_index.py` ne beneficia automaticamente. Nessuna nuova dipendenza; +2 test `capsys` → 54 PASS |
...
- Suite di test offline deterministica (54 PASS) eseguibile senza runtime/rete grazie al `FakeEmbedder`.
...
4. Validazione finale di onboarding: clone reale da GitHub su macchina del team (Windows) → README fino a `54 passed` (la simulazione locale di clone pulito è un buon proxy ma gira in ambiente già configurato).
...
---
*Fine audit tecnico. Ultimo aggiornamento: 2026-06-30 (git `main` @ `06db6a9`; alias `retrieve_context` + progress feedback, 54 test).*
